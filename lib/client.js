...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var zh = {
      ...
      evQuestion: "用户提问",
      evQuestionHint: "agent 向你提问等待回答时推送（含计划批准，不走审批通道）",
      evError: "Agent 出错",
      ...
      tplQuestionTitle: "用户提问 · 标题",
      tplQuestionBody: "用户提问 · 正文",
      tplErrorTitle: "Agent 出错 · 标题",
      tplErrorBody: "Agent 出错 · 正文",
      placeholders: "可用占位符",
      phHint: "{workspace} 工作区名 · {session} 会话短 ID · {summary} 最后助手输出摘要 · {tool} 工具名 · {reason} 审批原因 · {questions} 提问内容 · {error} 错误信息 · {kind} 结束类型 · {time} 时间 · {date} 日期",
      save: "保存",
      saving: "保存中…",
      test: "测试发送",
      testing: "发送中…",
    ...
    var en = {
      ...
      evQuestion: "User question",
      evQuestionHint: "Push when the agent asks you a question (incl. plan approval; not the approval channel)",
      templates: "Templates",
      ...
      tplQuestionTitle: "User question · title",
      tplQuestionBody: "User question · body",
      tplErrorTitle: "Agent error · title",
      tplErrorBody: "Agent error · body",
      placeholders: "Placeholders",
      phHint: "{workspace} workspace · {session} short session id · {summary} last assistant excerpt · {tool} tool name · {reason} approval reason · {questions} question text · {error} error message · {kind} end kind · {time} time · {date} date",
      save: "Save",
      saving: "Saving…",
      test: "Send test",
      testing: "Sending…",
    ...
    // brand-primary 在浅色模式下是近黑色、深色模式下是近白色（反转变量）——
    // 文字色必须用配套的 label-primary-foreground（浅=白/深=黑），固定 #ffffff 在深色模式下不可读
    var tabActiveStyle = { padding: "6px 16px", borderRadius: "9px", border: "none", background: "var(--dsw-alias-brand-primary)", color: "var(--dsw-alias-label-primary-foreground)", fontSize: 13, fontWeight: 600, cursor: "pointer", fontFamily: "inherit" };
    ...
    function WechatSection(props) {
      ...
      var renderTemplateFields = function (tpl, setter) {
        return [
          ...
          createElement(TemplateField, { key: "t6q1", label: t("tplQuestionTitle"), value: tpl.questionTitle, onChange: function (v) { setter("questionTitle", v); } }),
          createElement(TemplateField, { key: "t6q2", label: t("tplQuestionBody"), value: tpl.questionBody, onChange: function (v) { setter("questionBody", v); } }),
      ...
      return createElement("div", { style: sectionStyle },
        ...
        createElement("div", { style: cardStyle },
          ...
          createElement(ToggleRow, {
            checked: ev.question !== false,
            label: t("evQuestion"),
            hint: t("evQuestionHint"),
            onChange: function (v) { setEvent("question", v); },
          }),
          createElement(ToggleRow, {
